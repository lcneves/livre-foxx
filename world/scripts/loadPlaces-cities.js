...
for (let i = 0; i < 2; ++i) {
  files.forEach(function (file) {
    ...
    places.forEach(function (place) {
      ...
      if (fields[6] === 'P') {
        collection = 'cities';
        geonameId += fields[0]; // Ex: US/2938492
      } else if (fields[7] === 'ADM1') { 
        collection = 'adm1';
        geonameId += fields[10];
      } else if (fields[7] === 'ADM2') {
        collection = 'adm2';
        geonameId += fields[10] + '/' + fields[11];
      } else if (fields[7] === 'ADM3') {
        collection = 'adm3';
        geonameId += fields[10] + '/' + fields[11] + '/' + fields[12];
      } else if (fields[7] === 'ADM4') {
        collection = 'adm4';
        geonameId += fields[10] + '/' + fields[11] + '/' + fields[12]
          + '/' + fields[13];
      } else if (
        ...
        geonameId = fields[8];
      } else if (fields[7] === 'TERR' && (
            fields[8] === 'AS' ||
            fields[8] === 'EH' ||
            fields[8] === 'SJ'
            )) {
        /*
         * Since, for now, only American Samoa, Western Sahara and Svalbard are
         * territories that parent other places, let's put them in the
        ...
        collection = 'countries';
        geonameId = fields[8];
      } else return; // We're not interested in places of other types.
      ...
      if (i === 0) { // First pass, load vertices
        ...
        switch (collection) {
          case 'cities':
            ...
            else return; // City has no suitable parent administrative region
                         //(malformed dataset?)
            break;
          case 'adm1':
          ...
          case 'countries':
            ...
            break;
          case 'territories':
            toSearchTerms = ['countries', fields[9]];
            break;
        }
